...
class PopupWidget extends StatefulWidget {
  ...
  final Widget Function(Offset globalPosition, Size size,LayerLink layerLink) popupBuilder;
  PopupWidget({Key? key,required this.child,required this.popupBuilder, this.onHitTest}):assert(child.key == null ? throw "Popup widget child has no global or local key":true), super(key: key);
...
class _PopupWidgetState extends State<PopupWidget> {
  ...
  Widget build(BuildContext context) {
    return GestureDetector(
        ...
        onTap: (){
          if(widget.onHitTest == null || (widget.onHitTest != null && widget.onHitTest!())){
  ...
  _onWidgetTap(BuildContext context) {
    ...
    if( (widget.child.key as GlobalKey).currentContext != null && (widget.child.key as GlobalKey).currentContext?.findRenderObject() != null){
...
class PopupWidgetModal extends ModalRoute<void> {
  final Widget child;
  PopupWidgetModal({required this.child,});
  ...
  Widget buildPage(
    ...
    return GestureDetector(
      ...
      onPanStart: (x){Navigator.pop(context);},
      child: Material(
